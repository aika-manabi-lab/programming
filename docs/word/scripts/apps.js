...
function ShowWizard(start_index) {
  ...
  if (Wizards.length == 0) MakeWizard();
...
function CalcArea() {
  ...
  cell_size = Math.floor(
    (half_height - puzzle_margin_top * 2 - outer_size * 2) /
      CurrentPuzzle.row_count
  );
  let margin = Math.floor(
    (width - cell_size * CurrentPuzzle.column_count - outer_size * 2) / 2
  );
  ...
  if (margin_left < 0) {
    ...
    cell_size = Math.floor(
      (width - puzzle_margin_left - puzzle_margin_right - outer_size * 2) /
        CurrentPuzzle.column_count
    );
    margin_left = 0;
    margin_right = 0;
  }
  ...
  var puzzle_height =
    cell_size * CurrentPuzzle.row_count +
    puzzle_margin_top +
    puzzle_margin_bottom +
    outer_size * 2;
  ...
  if (puzzle_height + hint_height + info_height + palette_height > height) {
    ...
    cell_size = Math.floor(
      (h - puzzle_margin_top * 2 - outer_size * 2) / CurrentPuzzle.row_count
    );
    puzzle_height =
      cell_size * CurrentPuzzle.row_count +
      puzzle_margin_top * 2 +
      outer_size * 2;
    puzzle_margin_left = Math.floor(
      (width - cell_size * CurrentPuzzle.column_count - outer_size * 2) / 2
    );
...
function IsHintH(str) {
  for (var i = 0; i < CurrentPuzzle.hints.h.length; i++) {
    if (CurrentPuzzle.hints.h[i][1] == str) return true;
  }
  return false;
}
function IsHintV(str) {
  for (var i = 0; i < CurrentPuzzle.hints.v.length; i++) {
    if (CurrentPuzzle.hints.v[i][1] == str) return true;
  }
  return false;
}
...
function ParsePuzzle() {
  ...
  OuterRect.right =
    offset_x + cell_size * CurrentPuzzle.column_count + outer_size / 2;
  OuterRect.bottom =
    offset_y + cell_size * CurrentPuzzle.row_count + outer_size / 2;
      ...
      if (PuzzleCells[y][x].chr != "") {
        ...
        if (str.length >= 2 && IsHintH(str)) {
          ...
          if (PuzzleCells[y][start_x].marked == 0) {
            ...
            chunk.num = num++;
          } else chunk.num = PuzzleCells[y][start_x].marked;
          Chunks.push(chunk);
        }
        start_x = -1;
        str = "";
      ...
      if (PuzzleCells[y][x].chr != "") {
        ...
        if (str.length >= 2 && IsHintV(str)) {
          ...
          if (PuzzleCells[start_y][x].marked == 0) {
            ...
            chunk.num = num++;
          } else chunk.num = PuzzleCells[start_y][x].marked;
          Chunks.push(chunk);
        }
        start_y = -1;
        str = "";
...
$(function () {
  ...
  _target.on(_start, function (e) {
    var point = e.originalEvent.changedTouches
    ...
    $(document).on(_end, function (e) {
      if (e.target.id == "canvas_puzzle") {
        point = e.originalEvent.changedTouches
          ? e.originalEvent.changedTouches[0]
          : e;
...
function DrawChunk(chunk, answer, hilight) {
  ...
  for (var i = 0; i < chunk.str.length; i++) {
    ...
    ctx.fillStyle = "black";
    var cell;
    if (chunk.dir == 1) cell = PuzzleCells[y + i][x];
    else cell = PuzzleCells[y][x + i];
    ...
    if (chunk.dir == 1) pos_y += cell_size;
    else pos_x += cell_size;
  }
}
...
function DrawFocus() {
  ...
  if (Selected.chunk.dir == 1) j += Selected.index;
...
function EnterKey(chr) {
  ...
  DrawChunk(Selected.chunk, false, true);
  if (IsSolved()) {
    if (!IsFinish()) $("#correct").get(0).play();
    ...
    var newScore = CalcScore();
    if (orgScore != newScore) AddScore(newScore - orgScore);
    // フォーカスの移動
    if (Selected.index < Selected.chunk.str.length - 1) Selected.index++;
    DrawFocus();
  }
}
...
function IsFinish() {
  ...
  for (var j = 0; j < PuzzleCells.length; j++) {
    for (var i = 0; i < PuzzleCells[j].length; i++) {
      if (PuzzleCells[j][i].chr != PuzzleCells[j][i].input) return false;
    }
  }
  return true;
}
...
function SpeakHint() {
  ...
  Speech(ans, true);
  SpeakCount++;
  if (SpeakCount == 3 || SpeakCount == 5) CreateButtonPalette();
}

function IsRight(array) {
  if (array.length != Selected.chunk.str.length) return false;
  var right = true;
  for (var j = 0; j < Selected.chunk.str.length; j++) {
    ...
    if (event.resultIndex == 0) {
      for (var i = 0; i < event.results.length; ++i) {
        if (event.results[i].isFinal) {
          ...
          var orgScore = CalcScore();
          if (!IsRight(array)) {
            ons.notification.toast(text + "と聞こえました．", {
              timeout: 2000,
            });
            rec_result = 2;
            return;
          ...
          if (IsSolved()) {
            if (!bClear) $("#correct").get(0).play(); // 音が重なるのを防ぐ
            Selected.chunk.speak = 1;
            var newScore = CalcScore();
            AddScore(newScore - orgScore);
          } else {
            var newScore = CalcScore();
            if (newScore != orgScore) AddScore(newScore - orgScore);
          }
          if (bClear) {
            Studied.push(CurrentPuzzle.id);
            localStorage.setItem(Studied_name, JSON.stringify(Studied));
...
function StartRec() {
  if (recognition) {
    if ($("#palette_button").hasClass("active")) return;
    if (Selected.chunk) {
      ClearButtonPalette();
      $("#palette_desc")
        .css("font-size", "22px")
        .text("マイクに向かってしゃべってください．");
      recognition.start();
    } else {
      Speech("Please select any cell.", false);
    }
  } else {
    ons.notification.toast("この機種は音声認識をサポートしていません。", {
      timeout: 2000,
    });
  }
}
